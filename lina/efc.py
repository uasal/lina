...
def calibrate(I, 
    ...
    if return_all: response_cube = xp.zeros((I.npsf**2, 2, Nmodes))
    print('Calculating Jacobian: ')
    for i,dm_mode in enumerate(calibration_modes):
        response = 0
        for s in [-1, 1]:
            # Add the mode to the DMs
            I.add_dm(s * calibration_amp * dm_mode.reshape(I.Nact,I.Nact))
            # Compute/Measure reponse with model or SCC
            efield = I.calc_wf() if scc_fun is None else scc_fun(I, **scc_params)
    ...
    if plot:
        ...
        dm_response_map = xp.sqrt(xp.mean(xp.abs(dm_responses)**2, axis=0)).reshape(I.Nact, I.Nact)
...
def run(I, 
    ...
    if len(all_commands)>0:
        ...
        total_command = xp.zeros((I.Nact,I.Nact))
    ...
    for i in range(iterations):
        ...
        if est_fun is not None and est_params is not None:
            ...
            electric_field = I.calc_wf() # no PWP, just use model
        ...
        # del_command = calibration_modes.reshape(Nmodes, -1).T.dot(modal_coeff).reshape(I.Nact,I.Nact)
        ...
        if plot_current: 
            ...
            if plot_radial_contrast:
                utils.plot_radial_contrast(image, control_mask, I.psf_pixelscale_lamD, nbins=100)
